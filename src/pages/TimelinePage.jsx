...
import { Heart, Calendar, MapPin, User, Plus, Edit3, Trash2, Camera, Images } from 'lucide-react';
...
//   const {user, setUser} = useAuth()
  ...
  useEffect(() => {
    ...
    // setTimeout(() => {
    //     setMemories(mockMemories.sort((a, b) => new Date(b.date) - new Date(a.date)));
    //     setLoading(false);
    // }, 1000);
  }, [user, userDetails]);
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
        <div className="relative max-w-4xl max-h-full">
          <img
            src={selectedMemory.selectedImageIndex ? 
              selectedMemory.images[selectedMemory.selectedImageIndex] : 
          ...
          <div className="absolute bottom-4 left-4 text-white bg-black bg-opacity-50 px-4 py-2 rounded-lg">
            ...
            <p className="text-sm opacity-90">{formatDate(selectedMemory.date)}</p>
          </div>
        </div>
      </div>
    );
